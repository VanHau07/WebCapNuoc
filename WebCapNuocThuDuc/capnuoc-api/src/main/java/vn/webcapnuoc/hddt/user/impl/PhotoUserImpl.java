...
@Repository
@Transactional
public class PhotoUserImpl extends AbstractDAO implements PhotoDAO {
	...
	public MsgRsp getPhoto(JSONRoot jsonRoot) throws Exception {
		...
		cursor = mongoTemplate.getCollection("Photo").aggregate(pipeline).allowDiskUse(true);
	...
	public MsgRsp detail(JSONRoot jsonRoot, String _id) throws Exception {
		...
		Document docTmp = null;
		Iterable<Document> cursor = mongoTemplate.getCollection("Photo").find(docFind);
		...
		/*TANG ViewCount +1*/
		if(null != docTmp) {
			mongoTemplate.getCollection("Photo").findOneAndUpdate(
				docFind
				, new Document("$inc", 
