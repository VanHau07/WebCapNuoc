...
@RequestMapping({
	...
	, "/admin/blogDelete"
	, "/admin/blogHotnews"
	, "/admin/blogCheckHotnews"
	
	
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class BlogAdminCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(BlogAdminCRUDController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		return "admin/content/blog_Crud";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action, String transaction, String method) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/blog/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "blogCreate": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "blogEdit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		case "blogActive": actionCode = Constants.MSG_ACTION_CODE.ACTIVE; break;
		case "blogDeActive": actionCode = Constants.MSG_ACTION_CODE.DEACTIVE; break;
		case "blogDelete": actionCode = Constants.MSG_ACTION_CODE.DELETE; break;
		case "blogHotnews": actionCode = Constants.MSG_ACTION_CODE.HOTNEWS; break;
		case "blogCheckHotnews": actionCode = Constants.MSG_ACTION_CODE.CHECK_HOTNEWS; break;
		default:
			dtoRes = new BaseDTO();
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/blog/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
