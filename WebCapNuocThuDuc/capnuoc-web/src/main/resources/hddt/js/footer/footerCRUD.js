$(function(){
	...
	$("#fFooterCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fFooterCRUD').find('input[name="_id"]').val());			
			formData.append('description', encodeURIComponent(tinyMCE.editors["description"].getContent()));
			formData.append('copyright', $('#fFooterCRUD').find('input[name="copyright"]').val());
			    			            	...
			    			            	if(res){
			    			            		if(res.errorCode == 0) {
			    			            			...
			    			            			alertDLSuccess(jQuery.type(res.responseData) == 'object'? res.responseData.info: createObjectError(res).html(), function(){
			    			            				...
			    			            				$('#fFooterCRUD').find('button[data-action="back"]').trigger('click');
			    			            		}else{
...
//	            			});
		...
		case 'back':
					...
					if($('#fFooter').find('#grid').length > 0){
						try{
							$('#fFooter').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fFooterCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
