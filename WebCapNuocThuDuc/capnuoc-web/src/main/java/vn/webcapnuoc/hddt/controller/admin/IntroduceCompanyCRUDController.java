...
public class IntroduceCompanyCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(IntroduceCompanyCRUDController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "introCreate":
			header = "Thêm mới thông tin giới thiệu";
			action = "CREATE";
			isEdit = true;
			break;
		case "introDetail":
			header = "Chi tiết giới thiệu";
			action = "DETAIL";
			isEdit = false;
			break;
		case "introEdit":
			header = "Thay đổi nội dung giới thiệu";
			action = "EDIT";
			isEdit = true;
			break;
		default:
		...
		return "admin/content/IntroCompanyCRUD";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action, String transaction, String method) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/introduce/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	public BaseDTO checkDataToAccept(HttpServletRequest req, HttpSession session, String transaction
		...
		case "introCreate":
			...
			if("".equals(attachFileName) || "".equals(attachFileNameSystem)) {
				dto.setErrorCode(1);
				dto.getErrorMessages().add("Vui lòng kiểm tra lại ảnh đại diện.");
			}else {
				file = new File(SystemParams.DIR_TMP, attachFileNameSystem);
				if(!file.exists() || !file.isFile()) {
					dto.setErrorCode(1);
					dto.getErrorMessages().add("Tập tin ảnh đại diện không tồn tại.");
				}
			}
			if("".equals(content)) {
				dto.setErrorCode(1);
				dto.getErrorMessages().add("Vui lòng nhập vào nội dung.");
			}
			break;
		case "introEdit":
			if("".equals(_id)) {
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "introCreate": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "introEdit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		case "introActive": actionCode = Constants.MSG_ACTION_CODE.ACTIVE; break;
		case "introDeActive": actionCode = Constants.MSG_ACTION_CODE.DEACTIVE; break;
		case "introDelete": actionCode = Constants.MSG_ACTION_CODE.DELETE; break;
		case "introHotnews": actionCode = Constants.MSG_ACTION_CODE.HOTNEWS; break;
		default:
			dtoRes = new BaseDTO();
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/introduce/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
