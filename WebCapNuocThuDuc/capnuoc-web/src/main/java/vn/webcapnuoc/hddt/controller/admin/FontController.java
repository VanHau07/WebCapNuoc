...
public class FontController extends AbstractController {
	...
	public String main(Locale locale, Principal principal, HttpServletRequest request) throws Exception {
		request.setAttribute("title", "Kiểu chữ");
	...
	//	hData.put("Title", title);
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/font/list", cup.getLoginRes().getToken(),HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("LoaiFont", commons.getTextJsonNode(row.at("/LoaiFont")));
					hItem.put("LoaiFontText", commons.getTextJsonNode(row.at("/LoaiFontText")));
