...
public class BlogAdminController extends AbstractController {
...
//		LoadStatus(cup, locale, request, "");
	...
	//	hData.put("LoaiSach", loai_sach);
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/blog/list", cup.getLoginRes().getToken(),HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					String check_hot_new = commons.getTextJsonNode(row.at("/Check_HotNews"));
					String hot = "";
					String news = "";
