...
import vn.webcapnuoc.hddt.admin.dao.FontDAO;
...
@Repository
@Transactional
public class FontImpl extends AbstractDAO implements FontDAO {
	private static final Logger log = LogManager.getLogger(FontImpl.class);
	@Autowired
	...
	public MsgRsp crud(JSONRoot jsonRoot) throws Exception {
		...
		String loai_font = commons.getTextJsonNode(jsonData.at("/LoaiFont")).replaceAll("\\s", "");
		String loai_font_text = commons.getTextJsonNode(jsonData.at("/LoaiFontText")).replaceAll("\\s", "");
		...
		try {
			...
			case Constants.MSG_ACTION_CODE.CREATED:					
				...
				docTmp = new Document("Name", name)
						.append("LoaiFont", loai_font)			
						.append("LoaiFontText",loai_font_text)
				...
				mongoTemplate.getCollection("Font").insertOne(docTmp);
			...
			case Constants.MSG_ACTION_CODE.MODIFY:
				...
				cursor = mongoTemplate.getCollection("Font").find(docFind);
				...
				if(null == docTmp){
					...
					responseStatus = new MspResponseStatus(9999, "Loại kiểu chữ không tồn tại trong hệ thống.");
					rsp.setResponseStatus(responseStatus);
					return rsp;
				}
				
				if(docTmp.getBoolean("ActiveFlag", false)) {			
					responseStatus = new MspResponseStatus(9999, "Loại kiểu chữ đã được kích hoạt. Không thể thực hiện thay đổi.");
					rsp.setResponseStatus(responseStatus);
					return rsp;
				}
				...
				docTmp = mongoTemplate.getCollection("Font").findOneAndUpdate(
						docTmp
						, new Document("$set", 
			...
			case Constants.MSG_ACTION_CODE.HOTNEWS:
				...
				cursor = mongoTemplate.getCollection("Font").find(docFind);
				cursor1 = mongoTemplate.getCollection("Font").find(docFind1);
				docTmp = null;
				iter = cursor.iterator();
				if(iter.hasNext()) {
					docTmp = iter.next();
				...
				if((actionCode.equals(Constants.MSG_ACTION_CODE.ACTIVE) || actionCode.equals(Constants.MSG_ACTION_CODE.DELETE)) 
						&& activeFlag) {				
					responseStatus = new MspResponseStatus(9999, "Loại kiểu chữ này đã được kích hoạt");
					rsp.setResponseStatus(responseStatus);
					return rsp;
				}else if(actionCode.equals(Constants.MSG_ACTION_CODE.DEACTIVE) && !activeFlag) {				
					responseStatus = new MspResponseStatus(9999, "Loại kiểu chữ này chưa được kích hoạt.");
					rsp.setResponseStatus(responseStatus);
					return rsp;
				}
				...
				switch (actionCode) {
				case Constants.MSG_ACTION_CODE.ACTIVE:
					mongoTemplate.getCollection("Font").findOneAndUpdate(
							docFind
							, new Document("$set", 
	...
	public MsgRsp detail(JSONRoot jsonRoot, String _id) throws Exception {
		...
		Document docTmp = null;
		Iterable<Document> cursor = mongoTemplate.getCollection("Font").find(docFind);
	...
	public MsgRsp list(JSONRoot jsonRoot) throws Exception {
		...
		cursor = mongoTemplate.getCollection("Font").aggregate(pipeline).allowDiskUse(true);
		iter = cursor.iterator();
		if (iter.hasNext()) {
			docTmp = iter.next();
		}
		...
		if (null != rows) {
			for (Document doc : rows) {
				...
				hItem.put("LoaiFont", doc.get("LoaiFont"));
				hItem.put("LoaiFontText", doc.get("LoaiFontText"));			
