	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
                    url: ROOT_PATH + '/admin/main/title/search',
                    dataType: 'json',
...
//		height: kendoGridHeight,
        ...
        columns: [
			{field: 'STT', width: '60px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">STT</a>',
  				attributes: {'class': 'table-cell text-center'}, sortable: false, 
  				titleAttributes: {'class': 'table-title-cell text-center'}, template: "#= ++record #",
  			},
  			{field: 'Title', width: '200px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">Tiêu đề</a>',
				attributes: {'class': 'table-cell text-left'}, 
				titleAttributes: {'class': 'table-title-cell text-center'},
			},
			
				{field: 'Phone', width: '200px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">Số điện thoại</a>',
				attributes: {'class': 'table-cell text-left'}, 
				titleAttributes: {'class': 'table-title-cell text-center'},
			},
				{field: 'Email', width: '200px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">Email</a>',
				attributes: {'class': 'table-cell text-left'}, 
				titleAttributes: {'class': 'table-title-cell text-center'},
			},
			{field: 'StatusDesc', width: '120px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">Trạng thái</a>',
				attributes: {'class': 'table-cell text-left'}, 
				titleAttributes: {'class': 'table-title-cell text-center'},
			},
			{field: 'CreateDate', width: '130px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">Ngày tạo</a>',
				attributes: {'class': 'table-cell', style: 'text-align: center;'}, 
				titleAttributes: {'class': 'table-title-cell', style: 'text-align: center;',},
			},
			{field: 'CreateUserFullName', width: '130px', encoded: false, titleTemplate: '<a class="k-link" href="javascript:void(0);">Người tạo</a>',
		...
		dataBound: function(e) {
			...
			$("#ftitle").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', true);
			$("#ftitle").find('button[data-action="edit"]').prop('disabled', true);
			$("#ftitle").find('button[data-action="active"]').prop('disabled', true);
			$("#ftitle").find('button[data-action="de-active"]').prop('disabled', true);
			$("#ftitle").find('button[data-action="delete"]').prop('disabled', true);
		},
	});
	...
	_gridMain.find('table[role="grid"]').find('tbody').delegate('tr', 'click', function(e){
		...
		$("#ftitle").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', false);
		...
		$("#ftitle").find('button[data-action="edit"]').prop('disabled', !('N' == isActive) );
		$("#ftitle").find('button[data-action="active"]').prop('disabled', !('N' == isActive) );
		$("#ftitle").find('button[data-action="de-active"]').prop('disabled', 'N' == isActive );
		$("#ftitle").find('button[data-action="delete"]').prop('disabled', !('N' == isActive) );
	});
	
	$("#ftitle").find('button[data-action]').click(function (event) {
...
function getDataSearch(){
	var dataPost = {};
	dataPost['title'] = $("#ftitle").find('#title').val();
	return dataPost;
}