$(function(){
	...
	$("#fFontCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fFontCRUD').find('input[name="_id"]').val());
			formData.append('name', $('#fFontCRUD').find('#name').val());
			formData.append('loai-font', $('#fFontCRUD').find('#loai-font').val());
			formData.append('loai-font-text', $('#fFontCRUD').find('#loai-font').find('option:selected').text());		
			    			            	...
			    			            	if(res){
			    			            		if(res.errorCode == 0) {
			    			            			...
			    			            			alertDLSuccess(jQuery.type(res.responseData) == 'object'? res.responseData.info: createObjectError(res).html(), function(){
			    			            				...
			    			            				$('#fFontCRUD').find('button[data-action="back"]').trigger('click');
			    			            		}else{
...
//	            			});
		...
		case 'back':
					...
					if($('#fFont').find('#grid').length > 0){
						try{
							$('#fFont').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fFontCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
