	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
                    url: ROOT_PATH + '/admin/main/personnel/search',
                    dataType: 'json',
...
//		height: kendoGridHeight,
        ...
        columns: [
			{field: 'STT', width: '60px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">STT</a>',
  			...
  			{field: 'Name', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Tên nhân sự</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
			{field: 'ViTriText', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Vị trí nhân sự</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},		
				{field: 'Position', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Vị trí trong công ty</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
		...
		dataBound: function(e) {
			...
			$("#fPerson").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', true);
			$("#fPerson").find('button[data-action="edit"]').prop('disabled', true);
			$("#fPerson").find('button[data-action="active"]').prop('disabled', true);
			$("#fPerson").find('button[data-action="de-active"]').prop('disabled', true);
			$("#fPerson").find('button[data-action="delete"]').prop('disabled', true);
		},
	});
	...
	_gridMain.find('table[role="grid"]').find('tbody').delegate('tr', 'click', function(e){
		...
		$("#fPerson").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', false);
		...
		$("#fPerson").find('button[data-action="edit"]').prop('disabled', !('N' == isActive) );
		$("#fPerson").find('button[data-action="active"]').prop('disabled', !('N' == isActive) );
		$("#fPerson").find('button[data-action="de-active"]').prop('disabled', 'N' == isActive );
		$("#fPerson").find('button[data-action="delete"]').prop('disabled', !('N' == isActive) );
	});
	
	$("#fPerson").find('button[data-action]').click(function (event) {
...
function getDataSearch(){
	var dataPost = {};
	dataPost['name'] = $("#fPerson").find('#name').val();
	dataPost['vt-ns'] = $('#fPerson').find('#vt-ns').val();
	return dataPost;
}