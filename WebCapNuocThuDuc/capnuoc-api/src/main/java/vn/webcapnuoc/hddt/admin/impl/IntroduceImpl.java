...
import vn.webcapnuoc.hddt.admin.dao.IntroduceDAO;
...
@Repository
@Transactional
public class IntroduceImpl extends AbstractDAO implements IntroduceDAO {
	private static final Logger log = LogManager.getLogger(IntroduceImpl.class);
	@Autowired
	...
	public MsgRsp crud(JSONRoot jsonRoot) throws Exception {
		...
		try {
			...
			case Constants.MSG_ACTION_CODE.CREATED:		
				...
				mongoTemplate.getCollection("Introduce").insertOne(docTmp);
			...
			case Constants.MSG_ACTION_CODE.MODIFY:
				...
				cursor = mongoTemplate.getCollection("Introduce").find(docFind);
				...
				docTmp = mongoTemplate.getCollection("Introduce").findOneAndUpdate(
						docTmp
						, new Document("$set", 
			...
			case Constants.MSG_ACTION_CODE.HOTNEWS:
				...
				cursor1 = mongoTemplate.getCollection("Introduce").find(docFind1);							
				cursor = mongoTemplate.getCollection("Introduce").find(docFind);
				...
				case Constants.MSG_ACTION_CODE.ACTIVE:
					if(null != docTmp1){						
						responseStatus = new MspResponseStatus(9999, "Bài viết giới thiệu chỉ hiện 1 bài viết. vui lòng hủy kích hoạt bài viết trước đó!!!");
						rsp.setResponseStatus(responseStatus);
						return rsp;
					}
					mongoTemplate.getCollection("Introduce").findOneAndUpdate(
							docFind
							, new Document("$set", 
...
//					break;
	...
	public MsgRsp detail(JSONRoot jsonRoot, String _id) throws Exception {
		...
		Document docTmp = null;
		Iterable<Document> cursor = mongoTemplate.getCollection("Introduce").find(docFind);
	...
	public MsgRsp list(JSONRoot jsonRoot) throws Exception {
		...
		cursor = mongoTemplate.getCollection("Introduce").aggregate(pipeline).allowDiskUse(true);
		iter = cursor.iterator();
		if (iter.hasNext()) {
			docTmp = iter.next();
		}
