	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
                    url: ROOT_PATH + '/admin/main/equipment/search',
                    dataType: 'json',
...
//		height: kendoGridHeight,
        ...
        columns: [
			{field: 'STT', width: '60px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">STT</a>',
  			...
  			{field: 'HangMuc', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Hạng mục</a>',
		...
		dataBound: function(e) {
			...
			$("#fcsvc").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', true);
			$("#fcsvc").find('button[data-action="edit"]').prop('disabled', true);
			$("#fcsvc").find('button[data-action="active"]').prop('disabled', true);
			$("#fcsvc").find('button[data-action="de-active"]').prop('disabled', true);
			$("#fcsvc").find('button[data-action="delete"]').prop('disabled', true);
		},
	});
	...
	_gridMain.find('table[role="grid"]').find('tbody').delegate('tr', 'click', function(e){
		...
		$("#fcsvc").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', false);
		...
		$("#fcsvc").find('button[data-action="edit"]').prop('disabled', !('N' == isActive) );
		$("#fcsvc").find('button[data-action="active"]').prop('disabled', !('N' == isActive) );
		$("#fcsvc").find('button[data-action="de-active"]').prop('disabled', 'N' == isActive );
		$("#fcsvc").find('button[data-action="delete"]').prop('disabled', !('N' == isActive) );
	});
	
	$("#fcsvc").find('button[data-action]').click(function (event) {
...
function getDataSearch(){
	var dataPost = {};
	dataPost['hangmuc'] = $("#fcsvc").find('#hangmuc').val();
	return dataPost;
}