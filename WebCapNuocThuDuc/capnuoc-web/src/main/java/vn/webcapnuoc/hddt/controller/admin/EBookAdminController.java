...
public class EBookAdminController extends AbstractController {
...
//	}
	...
	public String main(Locale locale, Principal principal, HttpServletRequest request) throws Exception {
		request.setAttribute("title", "Quản lý sách");
...
//		LoadStatus(cup, locale, request, "");
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/ebook/list", cup.getLoginRes().getToken(),HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					String read_online = commons.getTextJsonNode(row.at("/ReadOnline"));
					String hot = "";
					String read = "";
						if(hot_new.equals("true")) {
								hot = "Y";
							}else {
								hot = "N";
							}
						if(read_online.equals("true")) {
							read = "Y";
						}else {
							read = "N";
						}
					...
					hItem.put("LoaiSach", commons.getTextJsonNode(row.at("/LoaiSach")));
					hItem.put("LoaiSachText", commons.getTextJsonNode(row.at("/LoaiSachText")));				
					hItem.put("SummaryContent", commons.getTextJsonNode(row.at("/SummaryContent")));			
					hItem.put("Price", commons.getTextJsonNode(row.at("/Price")));
					hItem.put("TacGia", commons.getTextJsonNode(row.at("/TacGia")));
					hItem.put("Year", commons.getTextJsonNode(row.at("/Year")));
					hItem.put("HotNewsDesc", hot);
					hItem.put("ReadOnline", read);
