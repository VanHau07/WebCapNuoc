$(function(){
	...
	$("#fIntroCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fIntroCRUD').find('input[name="_id"]').val());
			formData.append('title', $('#fIntroCRUD').find('#title').val());
			formData.append('content', encodeURIComponent(tinyMCE.editors["content"].getContent()));
			formData.append('attachFileName', $('#fIntroCRUD').find('input[name="attachFileName"]').val());
			formData.append('attachFileNameSystem', $('#fIntroCRUD').find('input[name="attachFileNameSystem"]').val());
			    						...
			    						if(4 == this.readyState && this.status == 200) {
			    			            	...
			    			            	if(res){
			    			            		if(res.errorCode == 0) {
			    			            			...
			    			            			$('#fIntroCRUD').find('button[data-action="back"]').trigger('click');
			    			            		}else{
...
//	            			});
		...
		case 'back':
					...
					if($('#fIntroCompany').find('#grid').length > 0){
						try{
							$('#fIntroCompany').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fIntroCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
