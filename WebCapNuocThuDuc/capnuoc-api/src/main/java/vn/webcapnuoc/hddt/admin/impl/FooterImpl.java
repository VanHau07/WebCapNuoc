...
import vn.webcapnuoc.hddt.admin.dao.FooterDAO;
...
@Repository
@Transactional
public class FooterImpl extends AbstractDAO implements FooterDAO {
	private static final Logger log = LogManager.getLogger(FooterImpl.class);
	@Autowired
	...
	public MsgRsp crud(JSONRoot jsonRoot) throws Exception {
		...
		String copyright = commons.getTextJsonNode(jsonData.at("/CopyRight")).replaceAll("\\s", " ");
		...
		try {
			...
			case Constants.MSG_ACTION_CODE.CREATED:		
				...
				docTmp = new Document("Description", description)				
				...
				mongoTemplate.getCollection("Footers").insertOne(docTmp);
			...
			case Constants.MSG_ACTION_CODE.MODIFY:
				...
				cursor = mongoTemplate.getCollection("Footers").find(docFind);
				...
				docTmp = mongoTemplate.getCollection("Footers").findOneAndUpdate(
						docTmp
						, new Document("$set", 
			...
			case Constants.MSG_ACTION_CODE.HOTNEWS:
				...
				cursor = mongoTemplate.getCollection("Footers").find(docFind);
				cursor1 = mongoTemplate.getCollection("Footers").find(docFind1);
				docTmp = null;
				iter = cursor.iterator();
				if(iter.hasNext()) {
					docTmp = iter.next();
				...
				case Constants.MSG_ACTION_CODE.ACTIVE:
					if(null != docTmp1){						
						responseStatus = new MspResponseStatus(9999, "Chỉ được kích hoạt 1 footer. Vui lòng hủy kích hoạt dữ liệu trước đó để thực hiện chức năng!!!");
						rsp.setResponseStatus(responseStatus);
						return rsp;
					}
					mongoTemplate.getCollection("Footers").findOneAndUpdate(
							docFind
							, new Document("$set", 
	...
	public MsgRsp detail(JSONRoot jsonRoot, String _id) throws Exception {
		...
		Document docTmp = null;
		Iterable<Document> cursor = mongoTemplate.getCollection("Footers").find(docFind);
...
//	
		...
		cursor = mongoTemplate.getCollection("Footers").aggregate(pipeline).allowDiskUse(true);
		iter = cursor.iterator();
		if (iter.hasNext()) {
			docTmp = iter.next();
		}
		...
		if (null != rows) {
			for (Document doc : rows) {
				...
				hItem.put("Description", doc.getString("Description"));	
				hItem.put("CopyRight", doc.getString("CopyRight"));
