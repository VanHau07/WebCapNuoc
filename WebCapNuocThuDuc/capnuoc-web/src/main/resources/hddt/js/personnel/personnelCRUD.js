$(function(){
	...
	$("#fPersonCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fPersonCRUD').find('input[name="_id"]').val());
			formData.append('name', $('#fPersonCRUD').find('#name').val());
			formData.append('vt-ns', $('#fPersonCRUD').find('#vt-ns').val());
			formData.append('vt-ns-text', $('#fPersonCRUD').find('#vt-ns').find('option:selected').text());		
			formData.append('position', $('#fPersonCRUD').find('#position').val());			
			formData.append('attachFileName', $('#fPersonCRUD').find('input[name="attachFileName"]').val());
			formData.append('attachFileNameSystem', $('#fPersonCRUD').find('input[name="attachFileNameSystem"]').val());		
			    			            	...
			    			            	if(res){
			    			            		if(res.errorCode == 0) {
			    			            			...
			    			            			alertDLSuccess(jQuery.type(res.responseData) == 'object'? res.responseData.info: createObjectError(res).html(), function(){
			    			            				...
			    			            				$('#fPersonCRUD').find('button[data-action="back"]').trigger('click');
			    			            		}else{
...
//	            			});
		...
		case 'back':
					...
					if($('#fPerson').find('#grid').length > 0){
						try{
							$('#fPerson').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fPersonCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
