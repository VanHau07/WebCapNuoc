...
@Controller
@RequestMapping("/admin/configEmail")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class ConfigMailServerController extends AbstractController {
	@Autowired RestAPIUtility restAPI;
	@RequestMapping(value = "/init", method = { RequestMethod.POST, RequestMethod.GET })
	public String main(Locale locale, Principal principal, HttpServletRequest request) throws Exception {
		request.setAttribute("title", "Cấu hình email server Admin");
		...
		return "admin/content/configmail";
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/configEmail/list", cup.getLoginRes().getToken(),HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("SmtpServer", commons.getTextJsonNode(row.at("/SmtpServer")));
					hItem.put("SmtpPort", commons.getTextJsonNode(row.at("/SmtpPort")));
					hItem.put("EmailAddress",  commons.getTextJsonNode(row.at("/EmailAddress")));
					hItem.put("EmailPassword",  commons.getTextJsonNode(row.at("/EmailPassword")));
