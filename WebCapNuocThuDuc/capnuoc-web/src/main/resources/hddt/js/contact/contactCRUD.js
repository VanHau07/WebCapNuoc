$(function(){
	...
	$("#fContactCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fContactCRUD').find('input[name="_id"]').val());
			formData.append('title', $('#fContactCRUD').find('#title').val());
			formData.append('description', $('#fContactCRUD').find('input[name="description"]').val());
			formData.append('position', $('#fContactCRUD').find('input[name="position"]').val());
			formData.append('phone', $('#fContactCRUD').find('input[name="phone"]').val());
			formData.append('email', $('#fContactCRUD').find('input[name="email"]').val());
			formData.append('map', $('#fContactCRUD').find('input[name="map"]').val());
			    			            	...
			    			            	if(res){
			    			            		if(res.errorCode == 0) {
			    			            			...
			    			            			alertDLSuccess(jQuery.type(res.responseData) == 'object'? res.responseData.info: createObjectError(res).html(), function(){
			    			            				...
			    			            				$('#fContactCRUD').find('button[data-action="back"]').trigger('click');
			    			            		}else{
...
//	            			});
		...
		case 'back':
					...
					if($('#fContact').find('#grid').length > 0){
						try{
							$('#fContact').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fContactCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
