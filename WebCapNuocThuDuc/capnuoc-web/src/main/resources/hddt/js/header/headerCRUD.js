$(function(){
	...
	$("#fHeaderCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fHeaderCRUD').find('input[name="_id"]').val());	
			formData.append('phone', $('#fHeaderCRUD').find('input[name="phone"]').val());			
			formData.append('email', $('#fHeaderCRUD').find('input[name="email"]').val());
			formData.append('address', $('#fHeaderCRUD').find('input[name="address"]').val());
			formData.append('phone_sp', $('#fHeaderCRUD').find('input[name="phone_sp"]').val());			
			formData.append('email_sp', $('#fHeaderCRUD').find('input[name="email_sp"]').val());
			formData.append('map', $('#fHeaderCRUD').find('input[name="map"]').val());
...
//	            			});
		...
		case 'back':
					...
					if($('#fHeader').find('#grid').length > 0){
						try{
							$('#fHeader').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fHeaderCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
