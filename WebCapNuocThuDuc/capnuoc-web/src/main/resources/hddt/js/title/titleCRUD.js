$(function(){
	...
	$("#fTitleCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fTitleCRUD').find('input[name="_id"]').val());
			formData.append('title_user', $('#fTitleCRUD').find('#title_user').val());		
			formData.append('title_admin', $('#fTitleCRUD').find('input[name="title_admin"]').val());			
...
//	            			});
		...
		case 'back':
					...
					if($('#fTitle').find('#grid').length > 0){
						try{
							$('#fTitle').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fTitleCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
