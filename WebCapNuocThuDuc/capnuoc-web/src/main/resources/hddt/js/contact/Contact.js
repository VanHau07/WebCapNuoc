	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
                    url: ROOT_PATH + '/admin/main/contact/search',
                    dataType: 'json',
...
//		height: kendoGridHeight,
        ...
        columns: [
			{field: 'STT', width: '60px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">STT</a>',
  			...
  			{field: 'Name', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Tên khách hàng</a>',
				...
				{field: 'Phone', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Số điện thoại</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
				{field: 'Email', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Email</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
			{field: 'Date', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Ngày gửi</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
		],
		...
		dataBound: function(e) {
			...
			$("#fContact").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', true);
			$("#fContact").find('button[data-action="edit"]').prop('disabled', true);
	
			//$("#fContact").find('button[data-action="delete"]').prop('disabled', true);
		},
	});
	...
	_gridMain.find('table[role="grid"]').find('tbody').delegate('tr', 'click', function(e){
		...
		$("#fContact").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', false);
		...
		$("#fContact").find('button[data-action="edit"]').prop('disabled', !('N' == isActive) );
	
		$("#fContact").find('button[data-action="delete"]').prop('disabled', false );
	});
	
	$("#fContact").find('button[data-action]').click(function (event) {
		event.preventDefault();/*event.stopPropagation();*/
		var dataAction = $(this).data('action');
		var dataController = $(this).data('controller');
		
		var actionCheck = '|detail|edit|delete|';
		var grid = _gridMain.data("kendoGrid");
    	var rowData = grid.dataItem(grid.select());
...
function getDataSearch(){
	var dataPost = {};
	dataPost['name'] = $("#fContact").find('#name').val();
	return dataPost;
}