...
import vn.webcapnuoc.hddt.admin.dao.TitleDAO;
...
@Repository
@Transactional
public class TitleImpl extends AbstractDAO implements TitleDAO {
	private static final Logger log = LogManager.getLogger(TitleImpl.class);
	@Autowired
	...
	public MsgRsp crud(JSONRoot jsonRoot) throws Exception {
		...
		String title_user = commons.getTextJsonNode(jsonData.at("/Title_user")).replaceAll("\\s", " ");
		String title_admin = commons.getTextJsonNode(jsonData.at("/Title_admin")).replaceAll("\\s", " ");
		...
		try {
			...
			case Constants.MSG_ACTION_CODE.CREATED:		
				...
				docTmp = new Document("Title_user", title_user)
						.append("Title_admin", title_admin)	
				...
				mongoTemplate.getCollection("Titles").insertOne(docTmp);
			...
			case Constants.MSG_ACTION_CODE.MODIFY:
				...
				cursor = mongoTemplate.getCollection("Titles").find(docFind);
				...
				docTmp = mongoTemplate.getCollection("Titles").findOneAndUpdate(
						docTmp
						, new Document("$set", 
								new Document("Title_user", title_user)
								.append("Title_admin", title_admin)		
							.append("InfoUpdated", 
...
//			case Constants.MSG_ACTION_CODE.HOTNEWS:
				...
				cursor = mongoTemplate.getCollection("Titles").find(docFind);
				cursor1 = mongoTemplate.getCollection("Titles").find(docFind1);
				docTmp = null;
				iter = cursor.iterator();
				if(iter.hasNext()) {
					docTmp = iter.next();
				...
				case Constants.MSG_ACTION_CODE.ACTIVE:
					if(null != docTmp1){						
						responseStatus = new MspResponseStatus(9999, "Chỉ được kích hoạt 1 tiêu đề. Vui lòng hủy kích hoạt dữ liệu trước đó để thực hiện chức năng!!!");
						rsp.setResponseStatus(responseStatus);
						return rsp;
					}
					mongoTemplate.getCollection("Titles").findOneAndUpdate(
							docFind
							, new Document("$set", 
	...
	public MsgRsp detail(JSONRoot jsonRoot, String _id) throws Exception {
		...
		Document docTmp = null;
		Iterable<Document> cursor = mongoTemplate.getCollection("Titles").find(docFind);
	...
	public MsgRsp list(JSONRoot jsonRoot) throws Exception {
		...
		docMatch.append("Title_user", new Document("$regex", commons.regexEscapeForMongoQuery(title_user)).append("$options", "i"));
		docMatch.append("Title_admin", new Document("$regex", commons.regexEscapeForMongoQuery(title_admin)).append("$options", "i"));
		...
		cursor = mongoTemplate.getCollection("Titles").aggregate(pipeline).allowDiskUse(true);
		iter = cursor.iterator();
		if (iter.hasNext()) {
			docTmp = iter.next();
		}
		...
		if (null != rows) {
			for (Document doc : rows) {
				...
				hItem.put("Title_user", doc.getString("Title_user"));
				hItem.put("Title_admin", doc.getString("Title_admin"));
