	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
                    url: ROOT_PATH + '/admin/main/asset/search',
                    dataType: 'json',
...
//		height: kendoGridHeight,
        ...
        columns: [
			...
			{field: 'LoaiTSText', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Loại tài sản</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},		
				{field: 'User_Asset', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Người có tài sản đấu giá</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
				{field: 'Price', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Giá khởi điểm</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
				{field: 'StatusText', width: '300px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Trạng thái</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
		...
		dataBound: function(e) {
			...
			$("#fAsset").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', true);
			$("#fAsset").find('button[data-action="edit"]').prop('disabled', true);
			$("#fAsset").find('button[data-action="active"]').prop('disabled', true);
			$("#fAsset").find('button[data-action="de-active"]').prop('disabled', true);
			$("#fAsset").find('button[data-action="delete"]').prop('disabled', true);
		},
	});
	...
	_gridMain.find('table[role="grid"]').find('tbody').delegate('tr', 'click', function(e){
		...
		$("#fAsset").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', false);
		...
		$("#fAsset").find('button[data-action="edit"]').prop('disabled', !('N' == isActive) );
		$("#fAsset").find('button[data-action="active"]').prop('disabled', !('N' == isActive) );
		$("#fAsset").find('button[data-action="de-active"]').prop('disabled', 'N' == isActive );
		$("#fAsset").find('button[data-action="delete"]').prop('disabled', !('N' == isActive) );
	});
	
	$("#fAsset").find('button[data-action]').click(function (event) {
...
function getDataSearch(){
	var dataPost = {};
	dataPost['title'] = $("#fAsset").find('#title').val();
	dataPost['loai-ts'] = $('#fAsset').find('#loai-ts').val();
	return dataPost;
}