$(function(){
	...
	$("#fLogoCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fLogoCRUD').find('input[name="_id"]').val());
			formData.append('title', $('#fLogoCRUD').find('#title').val());
			formData.append('attachFileName', $('#fLogoCRUD').find('input[name="attachFileName"]').val());
			formData.append('attachFileNameSystem', $('#fLogoCRUD').find('input[name="attachFileNameSystem"]').val());
...
//	            			});
		...
		case 'back':
					...
					if($('#fLogo').find('#grid').length > 0){
						try{
							$('#fLogo').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fLogoCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
