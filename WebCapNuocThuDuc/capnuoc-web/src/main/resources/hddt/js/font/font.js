	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
                    url: ROOT_PATH + '/admin/main/font/search',
                    dataType: 'json',
...
//		height: kendoGridHeight,
        ...
        columns: [
			{field: 'STT', width: '60px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">STT</a>',
  			...
  			{field: 'Name', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Tên</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},
			{field: 'LoaiFontText', width: '200px', encoded: false, headerTemplate: '<a class="k-link" href="javascript:void(0);">Loại kiểu chữ</a>',
				attributes: {'class': 'table-cell text-left'}, 
				headerAttributes: {'class': 'table-header-cell text-center'},
			},		
		...
		dataBound: function(e) {
			...
			$("#fFont").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', true);
			$("#fFont").find('button[data-action="edit"]').prop('disabled', true);
			$("#fFont").find('button[data-action="active"]').prop('disabled', true);
			$("#fFont").find('button[data-action="de-active"]').prop('disabled', true);
			$("#fFont").find('button[data-action="delete"]').prop('disabled', true);
		},
	});
	...
	_gridMain.find('table[role="grid"]').find('tbody').delegate('tr', 'click', function(e){
		...
		$("#fFont").find('button[data-action="detail"], button[data-action="hotnews"]').prop('disabled', false);
		...
		$("#fFont").find('button[data-action="edit"]').prop('disabled', !('N' == isActive) );
		$("#fFont").find('button[data-action="active"]').prop('disabled', !('N' == isActive) );
		$("#fFont").find('button[data-action="de-active"]').prop('disabled', 'N' == isActive );
		$("#fFont").find('button[data-action="delete"]').prop('disabled', !('N' == isActive) );
	});
	
	$("#fFont").find('button[data-action]').click(function (event) {
