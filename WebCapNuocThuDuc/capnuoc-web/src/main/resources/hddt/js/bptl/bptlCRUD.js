$(function(){
	...
	$("#fBPTLCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fBPTLCRUD').find('input[name="_id"]').val());
			formData.append('title', $('#fBPTLCRUD').find('#title').val());
			formData.append('thongtu', $('#fBPTLCRUD').find('#thongtu').val());
			formData.append('ghichu', $('#fBPTLCRUD').find('#ghichu').val());
			formData.append('attachFileName', $('#fBPTLCRUD').find('input[name="attachFileName"]').val());
			formData.append('attachFileNameSystem', $('#fBPTLCRUD').find('input[name="attachFileNameSystem"]').val());
...
//	            			});
		...
		case 'back':
					...
					if($('#fbptl').find('#grid').length > 0){
						try{
							$('#fbptl').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fBPTLCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
