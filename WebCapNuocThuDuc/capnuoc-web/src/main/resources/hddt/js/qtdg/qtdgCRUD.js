$(function(){
	...
	$("#fQtdgCRUD").find('button[data-action]').click(function (event) {
		...
		case 'accept':
			...
			formData.append('_id', $('#fQtdgCRUD').find('input[name="_id"]').val());			
			formData.append('title', $('#fQtdgCRUD').find('#title').val());
			formData.append('attachFileName', $('#fQtdgCRUD').find('input[name="attachFileName"]').val());
			formData.append('attachFileNameSystem', $('#fQtdgCRUD').find('input[name="attachFileNameSystem"]').val());
...
//	            			});
		...
		case 'back':
					...
					if($('#fQtdg').find('#grid').length > 0){
						try{
							$('#fQtdg').find('#grid').data("kendoGrid").dataSource.read();
						}catch(err){}
					}						
					break;
...
function setEventFileUpload(){	
	$("#fQtdgCRUD").find('input[type="file"][name="attachFile"]').fileupload({
		dataType: 'json',
